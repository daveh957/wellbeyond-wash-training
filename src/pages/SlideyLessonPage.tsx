...
const LessonDetailsPage: React.FC<LessonProps> = ({ history, subject,lesson, userLesson, isLoggedIn, trainerMode, updateLesson }) => {
  ...
  const slideNext = () => {
    ...
    s.getActiveIndex().then((idx:number) => {
      s.lockSwipeToNext(false);
      if (idx === 0 && userLesson.completed && lesson.questions && !trainerMode) {
        // Skip the "pre" questions
        s.slideTo(lesson.questions.length + 1, 0);
      }
      else {
        s.slideNext();
      }
        ...
        {lesson ?
          <IonContent ref={contentRef} fullscreen={true}>
            ...
            <IonSlides ref={slider} options={slideOpts} id={`${lesson.id}-slider`} onIonSlideDidChange={slideChanged}>
              ...
              {lesson.questions && lesson.questions.map((question:Question, idx:number) => {
                return (
